...
> = (props, ref) => {
	...
	const autocompleteRef = useRef<TAutocompleteStrategy | undefined>(
		undefined
	);
		...
		if (props.customControls) {
			props.customControls.forEach((control) => {
				if (control.type === "inline" && control.inlineStyle) {
					customStyleMap[control.name.toUpperCase()] =
						control.inlineStyle;
	...
	const handleAutocompleteSelected = (index?: number) => {
		...
		if (items.length > itemIndex) {
			...
			const offset =
				currentSelection.getFocusOffset() + searchTerm.length + 1;
	...
	const getAutocompleteItems = (): TAutocompleteItem[] => {
		...
		return autocompleteRef
			.current!.items.filter(
				(item) =>
					item.keys.filter((key) => key.includes(searchTerm)).length >
					0
			)
	...
	const keyBindingFn = (e: React.KeyboardEvent<{}>): string | null => {
		if (hasCommandModifier(e) && props.keyCommands) {
			const comm = props.keyCommands.find(
				(comm) => comm.key === e.keyCode
			);
			if (comm) {
				return comm.name;
			}
